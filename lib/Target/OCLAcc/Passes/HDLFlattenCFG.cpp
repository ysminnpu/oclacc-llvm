//===- HDLFlattenCFG.cpp - Implementation of FlattenCFG pass --------------===//
//===----------------------------------------------------------------------===//
  ...
  if (isHeadOfIfPattern(BB, &ifpattern) == true) {
    ...
    if ((switchpattern.canIfConvert(DL) == true)
     && (switchpattern.shouldIfConvert(*RE) == true)) {
      ...
      DEBUG(dbgs() << ">>Unconvertible unknown switch-pattern with head at "
...
bool Loopus::HDLFlattenCFGWorker::runOnFunction(Function *F) {
  ...
  do {
    ...
    DEBUG(dbgs() << "New DOMTree iteration...\n");
    ...
    for (po_iterator<DominatorTree*> DTIT = po_begin(DT), DTEND = po_end(DT);
        DTIT != DTEND; ++DTIT) {
      // DEBUG(dbgs() << "Domtreeit at " << DTIT->getBlock()->getName() << "\n");
...
bool Loopus::isHeadOfIfPattern(BasicBlock *BB, IfPattern *Pattern) {
  ...
  const TerminatorInst *BBTI = BB->getTerminator();
  ...
  const TerminatorInst *Succ0TI = Succ0->getTerminator();
  const TerminatorInst *Succ1TI = Succ1->getTerminator();
...
bool Loopus::isHeadOfSwitchPattern(BasicBlock *BB, SwitchPattern *Pattern) {
  ...
  for (SwitchInst::CaseIt CSIT = BBSI->case_begin(), CSEND = BBSI->case_end();
    ...
    TerminatorInst *CaseTI = CaseBB->getTerminator();
  ...
  if (BB2 != nullptr) {
    TerminatorInst *BB1TI = BB1->getTerminator();
    TerminatorInst *BB2TI = BB2->getTerminator();
  ...
  if (DefBB == TailBB) {
    ...
    const TerminatorInst *DefTI = DefBB->getTerminator();
...
bool Loopus::IfPattern::canIfConvert(const DataLayout *DL) const {
  ...
  // Get terminator for head basic block
  const TerminatorInst *HeadTI = HeadBB->getTerminator();
  ...
  if (HeadBI == nullptr) {
    DEBUG(dbgs() << "  Cannot convert as terminator of head BB is not a "
        << "BranchInst!\n");
    return false;
  }
  if (HeadBI->isConditional() == false) {
    DEBUG(dbgs() << "  Cannot convert pattern with non-conditional branch!\n");
    return false;
  }
  ...
  if (HeadNumSuccessors != 2) {
    DEBUG(dbgs() << "  Cannot convert as head BB does not exactly have two "
        << "successors!\n");
    return false;
  }
  ...
  if (isTriangle() == true) {
    ...
    const TerminatorInst *BranchedBBTI = BranchedBB->getTerminator();
    ...
    if (BranchedBBBI == nullptr) {
      DEBUG(dbgs() << "  BB in branch must end with a BranchInst!\n");
      return false;
    }
    if (BranchedBBBI->isUnconditional() == false) {
      DEBUG(dbgs() << "  BB in branch must end with an unconditional branch!\n");
      return false;
    }
    ...
    if (TBBBI == nullptr) {
      DEBUG(dbgs() << "  TBB must end with a BranchInst!\n");
      return false;
    }
    if (TBBBI->isUnconditional() == false) {
      DEBUG(dbgs() << "  TBB must end with an unconditional branch!\n");
      return false;
    }
    ...
    if (FBBBI == nullptr) {
      DEBUG(dbgs() << "  FBB must end with a BranchInst!\n");
      return false;
    }
    if (FBBBI->isUnconditional() == false) {
      DEBUG(dbgs() << "  FBB must end with an unconditional branch!\n");
      return false;
    }
  ...
  if (TBB != nullptr) {
    ...
    if (isa<PHINode>(TBB->front()) == true) {
      DEBUG(dbgs() << "  Cannot speculate TBB block with PHIs in it!\n");
      return false;
    }
    // Test if TBB can be speculated
    if (canExecuteSpeculatively(TBB, DL) == false) {
      DEBUG(dbgs() << "  Cannot speculatively execute BB " << TBB->getName()
          << "!\n");
      return false;
    }
  }
  if (FBB != nullptr) {
    ...
    if (isa<PHINode>(FBB->front()) == true) {
      DEBUG(dbgs() << "  Cannot speculate FBB block with PHIs in it!\n");
      return false;
    }
    // Test if FBB can be speculated
    if (canExecuteSpeculatively(FBB, DL) == false) {
      DEBUG(dbgs() << "  Cannot speculatively execute BB " << FBB->getName()
          << "!\n");
      return false;
    }
  }
  ...
  if (hasPHIs == false) {
    ...
    DEBUG(dbgs() << "  Cannot convert pattern as no PHIs in tail BB "
...
bool Loopus::IfPattern::shouldIfConvert(Loopus::RessourceEstimatorBase &RE) const {
  if (TBB != nullptr) {
    ...
    DEBUG(dbgs() << "    sz: tbb=" << TBBSize << "\n");
    if (TBBSize > MaxSpecBlockSize) {
      DEBUG(dbgs() << "  Will not speculate BB " << TBB->getName() << " as it "
          << "is too large...\n");
      return false;
    }
  }
  if (FBB != nullptr) {
    ...
    DEBUG(dbgs() << "        fbb=" << FBBSize << "\n");
    if (FBBSize > MaxSpecBlockSize) {
      DEBUG(dbgs() << "  Will not speculate BB " << FBB->getName() << " as it "
          << "is too large...\n");
      return false;
    }
  }
...
bool Loopus::IfPattern::adaptPhiOps(AliasAnalysis *AA) {
  ...
  TerminatorInst *HeadTI = HeadBB->getTerminator();
  if (HeadTI == nullptr) { return false; }
...
bool Loopus::IfPattern::replacePhis(AliasAnalysis *AA) {
  ...
  for (BasicBlock::iterator INSIT = TailBB->begin(), INSEND = TailBB->end();
    ...
    if (PN->getNumIncomingValues() != 2) {
      DEBUG(dbgs() << ">>Cannot replace PHIs in " << TailBB->getName() << ". "
  ...
  if (TBB != nullptr) {
    ...
    if ((TBBDTN != nullptr) && (TBBDTN != RTDTN) && (TBBDTN != HeadDTN)) {
      if (TBBDTN->getNumChildren() == 0) {
        ...
        DEBUG(dbgs() << "  BB " << TBB->getName() << " must not have any "
  ...
  if (FBB != nullptr) {
    ...
    if ((FBBDTN != nullptr) && (FBBDTN != RTDTN) && (FBBDTN != HeadDTN)) {
      if (FBBDTN->getNumChildren() == 0) {
        ...
        DEBUG(dbgs() << "  BB " << FBB->getName() << " must not have any "
            << " children in DOM tree.\n");
      }
    }
  }
...
bool Loopus::IfPattern::doIfConversion(DominatorTree *DT, AliasAnalysis *AA,
  ...
  if (TBB != nullptr) {
    ...
    DEBUG(dbgs() << "  spex: tbb=true\n");
    ++StatsNumBlocksSpeculated;
  }
  if (FBB != nullptr) {
    HeadBB->getInstList().splice(HeadBB->getTerminator(), FBB->getInstList(),
        FBB->begin(), std::prev(FBB->end()));
    DEBUG(dbgs() << "  spex: fbb=true\n");
    ++StatsNumBlocksSpeculated;
  }
  ...
  if ((TailBB != nullptr) && (TailBB->getSinglePredecessor() == HeadBB)) {
    ...
    if (MergeBlockIntoPredecessor(TailBB, P) == false) {
      DEBUG(dbgs() << ">>Could not merge block " << TailBB->getName() << " "
...
bool Loopus::SwitchPattern::canIfConvert(const DataLayout *DL) const {
  ...
  const TerminatorInst* const HeadTI = HeadBB->getTerminator();
  assert(HeadTI != nullptr && "Head BB does not have a valid terminator!");
  if (HeadTI == nullptr) { return false; }
  ...
  for (SwitchInst::ConstCaseIt CSIT = HeadSI->case_begin(),
    ...
    if (CaseBB != TailBB) {
      ...
      const TerminatorInst* const CaseTI = CaseBB->getTerminator();
  ...
  if (hasDedicatedDefault() == true) {
    const TerminatorInst* const DefTI = DefBB->getTerminator();
    if (DefTI == nullptr) { return false; }
  ...
  for (SwitchInst::ConstCaseIt CSIT = HeadSI->case_begin(),
    ...
    if (canExecuteSpeculatively(CaseBB, DL) == false) {
      DEBUG(dbgs() << "  Cannot speculatively execute BB " << CaseBB->getName()
          << "!\n");
      return false;
    }
  }
  if ((DefBB != nullptr) && (hasDedicatedDefault() == true)) {
    if (canExecuteSpeculatively(DefBB, DL) == false) {
      DEBUG(dbgs() << "  Cannot speculatively execute BB " << DefBB->getName()
          << "!\n");
      return false;
    }
  }
...
bool Loopus::SwitchPattern::shouldIfConvert(Loopus::RessourceEstimatorBase &RE) const {
  ...
  for (SwitchInst::ConstCaseIt CSIT = HeadSI->case_begin(),
    ...
    if (CBBSize > MaxSpecBlockSize) {
      DEBUG(dbgs() << "  Will not speculate BB " << CaseBB->getName() << " as "
          << "it is too large...\n");
      return false;
    }
  }
  ...
  if (hasDedicatedDefault() == true) {
    ...
    if (DBBSize > MaxSpecBlockSize) {
      DEBUG(dbgs() << "  Will not speculate BB " << DefBB->getName() << " as "
          << "it is too large...\n");
      return false;
    }
  }
...
bool Loopus::SwitchPattern::replacePhis(ValueToValueMapTy &CaseCmpMap, AliasAnalysis *AA) {
  ...
  DEBUG(dbgs() << "   phi: removed=" << CountPHIsRemoved << "\n");
...
void Loopus::SwitchPattern::updateDomTree(DominatorTree *DT,
  ...
  for (BasicBlock *BB : removedBlocks) {
    ...
    if (BBDTN->getNumChildren() == 0) {
      ...
      DEBUG(dbgs() << "  BB " << BB->getName() << " must not have any "
          << " children in DOM tree.\n");
    }
  }
...
bool Loopus::SwitchPattern::doIfConversion(DominatorTree *DT, AliasAnalysis *AA,
  ...
  DEBUG(dbgs() << "  stat: injecting compares...\n");
  ...
  for (SwitchInst::CaseIt CSIT = HeadSI->case_begin(),
    ...
    HeadBB->getInstList().splice(HeadSI, CaseBB->getInstList(), CaseBB->begin(),
        std::prev(CaseBB->end()));
  ...
  if (hasDedicatedDefault() == true) {
    ...
    HeadBB->getInstList().splice(HeadSI, DefBB->getInstList(), DefBB->begin(),
        std::prev(DefBB->end()));
    ++CountBlocksSpeculated;
    ++StatsNumBlocksSpeculated;
  }
  DEBUG(dbgs() << "  spex: count=" << CountBlocksSpeculated << "\n");
